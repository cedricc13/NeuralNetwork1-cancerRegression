...
def get_paths(lr: float) -> dict:
    ...
    prefix = f"dnn40_8_lr_{_lr_tag(lr)}"
    return {
...
def train_model(learning_rate: float) -> dict:
    paths = get_paths(learning_rate)
    pp = prepare_data(paths['data_csv'])
    model = build_model(pp.X_train_scaled.shape[1], learning_rate)

    early = keras.callbacks.EarlyStopping(monitor='val_loss', patience=100, restore_best_weights=True)
    max_epochs = 1500
    history = model.fit(
...
def test_model(model_path: str | None = None) -> dict:
    paths = get_paths()
