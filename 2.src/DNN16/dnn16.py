...
class ConsoleLossLogger(keras.callbacks.Callback):
    """Print compact epoch progress lines to stdout."""
...
# Import PreProcessing (works when running this file directly)
try:
...
def build_model(input_dim: int, learning_rate: float) -> keras.Sequential:
    model = keras.Sequential([
        layers.Dense(ARCH[0], activation='relu', input_shape=(input_dim,)),
        layers.Dropout(0.1),
...
def train_model(learning_rate: float, data_csv: str | None = None) -> dict:
    ...
    if np.isnan(y_train_pred).any() or np.isnan(y_val_pred).any() or np.isnan(y_test_pred).any():
        raise ValueError("NaN predictions encountered. Try reducing learning rate or check preprocessing.")
    ...
    metrics = {
        ...
        'n_features': int(pp.X_train_scaled.shape[1]),
        'n_train': int(pp.X_train_scaled.shape[0]),
        'n_val': int(pp.X_val_scaled.shape[0]),
        'n_test': int(pp.X_test_scaled.shape[0]),
    }
...
def test_model(model_path: str | None = None, data_csv: str | None = None) -> dict:
    ...
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Model not found at {model_path}. Please run the script to train first.")
